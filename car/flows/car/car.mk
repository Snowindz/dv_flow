...
# Snowind: added for automation
sub_mks	=
#sub_mks += compatibility
sub_mks	+= tools common_libs 
sub_mks += abv_libs abv
sub_mks	+= hdl 
sub_mks += dut_libs dut 
sub_mks	+= elab_dut
...
# TBD
#sub_mks	+= test
#doxygen, no need; sub_mks	+= tb_doc
#DVT flow, no need: sub_mks	+= tb_lint
#DVT flow, no need: sub_mks	+= sandbox_ide

sub_mks	+= sim_top

#sub_mks += aldec.mk -- no need
#Synopsys QBUILD, no need: sub_mks += synopsys 

# ifeq hvllnk_in_dut,0
sub_mks	+= misc

sub_mks	+= elab


#TBD sim_mks += design_dump_info

#TBX: sim_mks += hvl_link
#TBX: sim_mks += hvl_link_indut

# ifeq hvllnk_in_dut,1
# sub_mks	 += misc

# hlv flow: eclipse/java/c/c++ HLV Model
# sub_mks	 += hlverif
...
# ICE flow: no need
# FSIM flow: no need 
# IFV UNR coverage flow: unr.mk
# VCS UNR coverage flow: vcs_unr.mk
# MTI qverify coverage flow: covercheck.mk

# pwr caclulator 



# $(call inc_sub_mk,<makefileName>)
define inc_sub_mk
__ACCER_$(call uc,$1)_MK	:=   ${__FLOW_DIR}/$1.mk  
include ${ __ACCER_$(call uc,$1)_MK }
endef

$(foreach sub_mk, ${sub_mks}, $(call inc_sub_mk,$sub_mk))
